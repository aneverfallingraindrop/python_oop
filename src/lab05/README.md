...
1. По имени 
>`sort_by_student_name()` вызывает `sort_by(key=lambda st: st.student_name)` и извлекает имя из каждого студента                   
>`sorted()` сравнивает имена в алфавитном порядке                     
>Возвращается новая коллекция с отсортированными студентами                 

2. По курсу (sort_by_course)

3. По среднему баллу (sort_by_gpa)
>GPA от низкого к высокому

![Картинка 1](./../../images/lab05/image01.png)
...
> Для каждого студента вызывает `filter_last_course(student)`                
...
![Картинка 1](./../../images/lab05/image02.png)
...
![Картинка 1](./../../images/lab05/image03.png)

### Сценарий 4: Паттерн «Стратегия» через callable-объекты
Показывает:

1. Callable-объекты как стратегии (StrategyByCourse, StrategyByName, StrategyByGPA)
Цепочка операций:
  - filter_by(filter_last_course) — оставляем только студентов 6-го курса (Петров, Смирнов)
  - sort_by_student_name() — сортируем по фамилии (Петров, Смирнов)
  - apply(StrategyByCourse()) — применяем стратегию к каждому
...
![Картинка 1](./../../images/lab05/image04.png)